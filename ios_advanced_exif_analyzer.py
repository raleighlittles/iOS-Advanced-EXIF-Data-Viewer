...
if __name__ == "__main__":
    ...
    argparse_parser.add_argument("-i", "--input-file", type=str, help="iPhone image filename")
    ...
    try:
        ...
        accel_plot_filename = accelerometer_visualizer.create_accelerometer_plot(image_basename, accel_data)

    except KeyError:
        print("[ERROR] Image is missing accelerometer data. Make sure image is a native iOS photo.")

    try:
        subject_region = exif_reader.get_focus_area(exif_data)
        print("[DEBUG] Focus region for image ", subject_region)
        subject_area.draw_subject_highlight(image_filepath, subject_region, img_width, img_height)
